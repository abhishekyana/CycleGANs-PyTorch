...
parser.add_argument('--dataroot', type=str, default='./young2old/', help='root directory of the dataset')
...
parser.add_argument('--continued', action='store_true', help='helps continuing a broken training')
...
if opt.continued:
	print("continueing from a broken position")
...
DATATA=ImageDataset(opt.dataroot, transforms_=transforms_, unaligned=True)
print(opt.batchSize,opt.n_cpu,len(DATATA),opt.dataroot)
dataloader = DataLoader(DATATA, batch_size=opt.batchSize, shuffle=True, num_workers=opt.n_cpu)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    with open("EpochVerify.txt",'w') as ffff:
    	ffff.write("\n"+str(epoch))
###################################
